...
void print_usage(char const *program_name) {
  ...
  // clang-format off
    ...
    ss << "  -o <path>             Path to output csv file (default: results.csv)" << "\n";
    ss << "  --maxPly              Number of plies out of book to record search depth for (default: 2)" << "\n";
...
int main(int argc, char const *argv[]) {
  ...
  std::string regex_engine, regex_book, filename = "results.csv";
  int max_ply = 2;
  ...
  if (cmd.has_argument("--maxPly")) {
    max_ply = std::stoi(cmd.get_argument("--minDepth"));
  }
  ...
  std::cout << "\nSaved " << date_map.size() << " data points from "
